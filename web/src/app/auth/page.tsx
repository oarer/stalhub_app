...
export default function Page() {
	...
	const handlePasswordLogin = async (event: React.FormEvent) => {
		...
		try {
			...
			const isGuest = user.roles?.some((role) => role.name === "clan_guest");
	...
	return (
		<section className="flex min-h-dvh w-full items-center justify-center px-4 py-8">
			<div className="flex w-full max-w-lg flex-col gap-5">
				...
				<div className="flex flex-col gap-6 rounded-xl bg-card p-6 ring-2 ring-primary/50">
					<div className="flex flex-col gap-2">
						...
						<p className="text-center text-muted-foreground text-xs">
							{t("auth.loginThroughWebsiteDesc")}
						</p>
					</div>
				...
				<p className={`${montserrat.className} font-semibold text-sm text-text-accent`}>
					{t("auth.terms")}{" "}
