...
export type RegionKey = 'items' | 'player' | 'status'
export type Regions = Record<RegionKey, Selection | null>

export type TradeStatus = 'idle' | 'agreement' | 'success' | 'unknown'

// Trade windows mark the pending agreement with a separator ("---") and the
// finished deal with a confirmation ("Успешно!"/success). Detection is loose so
// OCR noise (caps, stray characters) cannot hide an actual deal.
export function parseTradeStatus(text: string): TradeStatus {
	const value = normalize(text)
	if (!value) return 'idle'
	if (/успешно|succes|изготовлан|сделано/i.test(value)) return 'success'
	if (/^[—\s\-_|]{1,8}$/.test(value) || /согласие|agreement/i.test(value))
		return 'agreement'
	return 'unknown'
}
