...
http.Server.prototype.listen = function (
	...
	if (!nextServer) {
		...
		this.emit = function (
			...
			if (event === "request") {
				const request = values[0] as http.IncomingMessage;
				const response = values[1] as http.ServerResponse;
				if (request.headers["x-stalhub-capability"] !== capability) {
					response.writeHead(403, { "Content-Type": "text/plain" });
					response.end("Forbidden local client");
					return true;
				}
				delete request.headers["x-stalhub-capability"];
			}
			return (originalEmit as (...args: unknown[]) => boolean).apply(this, [
				event,
				...values,
			]);
		};
